  ...
  return (
    <div className='grid grid-cols-3 gap-4'>
      {backgroundEffects.map(effect => {
        return <Button onClick={() => handleClick(effect.value)} variant={"outline"} className={cn("flex flex-col items-center gap-2 py-10", effectsData?.config?.backgroundEffects === effect.value && "bg-muted")}>
                  <effect.icon />
                  {effect.label}
              </Button>
      })}
